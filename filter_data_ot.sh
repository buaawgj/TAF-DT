...
# Define the types of target environments
ENV_TYPES=("kinematic" "morph")

# Define the dataset levels
DATASET_LEVELS=("medium" "medium_expert")

# Define the OT step type for computing the cost
OT_STEP_TYPE="transition"
    ...
    for srctype in "${SRCTYPES[@]}"; do
        for env_type in "${ENV_TYPES[@]}"; do
            for level in "${DATASET_LEVELS[@]}"; do
                ...
                timeout 900 python "$FILE_NAME" --env "$env" --srctype "$srctype" --tartype "$tartype" --seq_len "$seq_len" --batch_size "$batch_size" --ot_step_type "$OT_STEP_TYPE" --dir "$DIR_PATH"
                exit_code=$?
